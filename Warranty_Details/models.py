...
class Warranty_Details(core_models.TimeStampedModel):
    ...
    report_number = models.ForeignKey(
        hull_report_models.Hull_Report, on_delete=models.CASCADE
    )
    ...
    return_product = models.CharField(
        max_length=3, choices=YesNo.choices, default=YesNo.NO
    )
    ...
    importance = models.CharField(
        max_length=8, choices=Importances.choices, default=Importances.DD
    )

    remark = models.CharField(
        max_length=20, choices=Remarks.choices, default=Remarks.WMR
    )

    compensation = models.DecimalField(decimal_places=2, max_digits=6, default=0)

    unit = models.CharField(
        max_length=3, choices=MonetaryUnits.choices, blank=True, null=True
    )
